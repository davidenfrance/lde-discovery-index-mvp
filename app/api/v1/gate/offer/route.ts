...
export async function POST(req: NextRequest) {
  try {
    ...
    const offer_id = "gate-" + randomUUID();
    ...
    return NextResponse.json({
      ...
      note: "Offer under FS-GATE-1.0. This is not an LDI query. Receipt is released only after accept-mvp. Stand-in fee is not GENIUS USD.",
    });
  } catch (err) {
    const message = err instanceof Error ? err.message : "gate_offer_failed";
